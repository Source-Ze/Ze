...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @UI_XB 
'''
...
keyboard = [
  ...
  [
    Button.url("𝗭𝗧𝗵𝗼𝗻™ 𓅛", "https://t.me/UI_XB")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس  زد إي ❤️** \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس  زد إي ❤️**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك  زد إي.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond(f"**- ارسل هذا الخطأ الى @UP_UO \n- الخطـأ** str(e)\n")
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("قنـاة السـورس", "https://t.me/UI_XB")
        ]
    ]
